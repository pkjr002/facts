...
def plot(search_terms1,path20k,search_terms2,path2k,loc=0,worldmap=0):
    ...
    d20k = xtract_data_4m_nc((path20k + data20k), 'sea_level_change', loc, 2020, 2150)
    slc20k = d20k['slc']
    time20k = d20k['time']
    lat_lon20k = [item.item() for item in [d20k['lat'], d20k['lon']]]

    d2k = xtract_data_4m_nc((path2k + data2k), 'sea_level_change', loc, 2020, 2150)
    slc2k = d2k['slc']
    time2k = d2k['time']
...
def plot_qqplot(time20k, slc20k, slc2k, data20k, data2k,lat_lon20k,lat_lon2k):
    ...
    for yr0, yr1 in enumerate(time20k):
        ...
        if yr1 == 2100 or yr1 == 2150:
            ...
            #
            ax.set_xlabel('PK (m)', fontsize=8)
            ax.set_ylabel('GGG (m)', fontsize=8)
            ...
            ax.text(0.05, 0.95, table_text, transform=ax.transAxes, verticalalignment='top', fontsize='7', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
            #
            axno += 1   
            #
    # TITLE Data
...
def xtract_data_4m_nc(dataNC,var,loc,yrST,yrEN):    
    ...
    #
    slc = data[var][:,idx_year,loc].values /1000
