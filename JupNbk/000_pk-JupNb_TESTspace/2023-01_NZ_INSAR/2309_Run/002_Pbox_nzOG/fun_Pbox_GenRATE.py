...
workflows = ["wf_1e","wf_1f","wf_2e","wf_2f","wf_3e", "wf_3f", "wf_4"]
#
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
total = {workflow: [f'total.workflow.{workflow[:2]}{workflow[3:]}.{region}.nc'] for workflow in workflows}
#
def filename(components):
    return f"{components}_{region}sl.nc"
#
local_file = [f'NZInsarGPS.NZInsarGPS.verticallandmotion_{region}sl.nc'] if region == 'local' else []
#
components_dict = {
...
components = {key: [filename(p) for p in files] + common_files + local_file +total.get(key, []) for key, files in components_dict.items()}
# ^^^
#
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def pbox_files(prefixes):
    return [f"{prefix}_{region}sl.nc" for prefix in prefixes] + common_files

prefixes_dict = {
    "pb_1e": ['emuAIS.emulandice.AIS', 'larmip.larmip.AIS', 
				'emuGrIS.emulandice.GrIS', 'emuglaciers.emulandice.glaciers'],
	#			
    "pb_1f": ['ar5AIS.ipccar5.icesheets_AIS', 'larmip.larmip.AIS', 
				'GrIS1f.FittedISMIP.GrIS_GIS','ar5glaciers.ipccar5.glaciers'],
	#
    "pb_2e": ['emuAIS.emulandice.AIS', 'larmip.larmip.AIS', 'deconto21.deconto21.AIS_AIS',
        		'bamber19.bamber19.icesheets_AIS', 'emuGrIS.emulandice.GrIS', 'bamber19.bamber19.icesheets_GIS',
        		'emuglaciers.emulandice.glaciers', 'ar5glaciers.ipccar5.glaciers'],
	#
    "pb_2f": ['ar5AIS.ipccar5.icesheets_AIS', 'larmip.larmip.AIS', 'deconto21.deconto21.AIS_AIS',
        		'bamber19.bamber19.icesheets_AIS', 'GrIS1f.FittedISMIP.GrIS_GIS', 'bamber19.bamber19.icesheets_GIS',
        		'emuglaciers.emulandice.glaciers', 'ar5glaciers.ipccar5.glaciers'],
}
pBox = {key: pbox_files(prefixes) for key, prefixes in prefixes_dict.items()}
#
# Function block
#
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
def FOcr8(path, folder_name):
...
def main(infiles, outfile, pyear_start=2020, pyear_end=2300, pyear_step=10):
...
def GenerateRateFiles(indir, tempdir, chunksize, outdir=None):
	...
	for this_file in infiles:
		...
		#
		# Setup the temporary directory for this rate file
		with tempfile.TemporaryDirectory() as file_tempdir:
			# Generate the rate file for this input file
			GenerateRateFile_dev(this_file, file_tempdir, chunksize, outfile)
	return(None)
