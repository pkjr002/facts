...
facts_dir=$(pwd)
#
EXP_FOLDER="experiments-pkjr002";   EXP_MAIN="NZrerun";     EXP=("NZrerun1.ssp585c")
...
#
source_file="latlon_basedON_NZ_with_islands_VLM-v2_.txt"
#
...
for exp in "${EXP[@]}"; do
  ...
  if [[ "$FULL_LOCATION_LOOP" -eq 0 ]]; then
    ...
    for ((i=0; i<total_lines; i+=lines_per_iteration)); do
      ...
      echo -e " ______________________________________________________________ " 2>&1 | tee -a "$facts_dir/$fileOUT"
      echo -e "==> begin loop $i ... " 2>&1 | tee -a "$facts_dir/$fileOUT"
      start=$(date +%s)
      echo "Started at:: $(date --date=@$start)" 2>&1 | tee -a "$facts_dir/$fileOUT"
      echo -e "experiment NAME:: $exp  |:|  experiment folder:: $EXP_FOLDER" 2>&1 | tee -a "$facts_dir/$fileOUT"
      echo -e "Source Location File:: $source_file |:| Temp Location File $target_file" 2>&1 | tee -a "$facts_dir/$fileOUT"
      echo -e "Chunk:: line $i T line $lines_copied \n" 2>&1 | tee -a "$facts_dir/$fileOUT"
      #
      cd $facts_dir
...
echo -e "EXPERIMENT subFOLDER:: "$EXP 2>&1 | tee -a "$facts_dir/$fileOUT"
