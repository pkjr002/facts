...
''' InsarGPS_postprocess_verticallandmotion.py

This runs the post-processing stage for the vertical land motion component of the InsarGPS
...
def InsarGPS_postprocess_verticallandmotion(nsamps, rng_seed, locationfilename, baseyear, pyear_start, pyear_end, pyear_step, chunksize, pipeline_id):
	...
	local_sl = np.around(local_sl)
	local_sl = local_sl.astype(np.int16)

	# Create the xarray data structures for the localized projections
	ncvar_attributes = {"description": "Local SLR contributions from vertical land motion according to InsarGPS workflow",
			"history": "Created " + time.ctime(time.time()),
			"source": "SLR Framework: InsarGPS workflow",
			"scenario": "NA",
			"baseyear": baseyear}
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description="Run the post-processing stage for the InsarGPS vertical land motion workflow",\
	epilog="Note: This is meant to be run as part of the InsarGPS module within the Framework for the Assessment of Changes To Sea-level (FACTS)")
	...
	InsarGPS_postprocess_verticallandmotion(args.nsamps, args.seed, args.locationfile, args.baseyear, args.pyear_start, args.pyear_end, args.pyear_step, args.chunksize, args.pipeline_id)
